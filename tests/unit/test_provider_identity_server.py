"""Server-level tests for Phase 7 provider identity routing.

These tests run the Starlette app with provider YAML files on disk and assert
against the recorded outbound request host. They cover the serving boundary:
identity resolution, fail-closed unresolved identity, and Phase 5 per-agent
passthrough behavior after provider assembly.
"""
...
def _provider_payload(group: str, *, upstream: str) -> dict[str, Any]:
    ...
    tools = {
        ...
        "codex": {"exec_command": "fast", "apply_patch": "premium"},
...
def _write_config_tree(tmp_path: Path, config_yaml: str) -> None:
    ...
    payloads = {
        ...
        "codex": _provider_payload("codex", upstream="https://api.openai.com"),
...
def test_header_resolves_codex_for_responses(
    client: TestClient,
    upstream: Upstream,
) -> None:
    response = client.post(
        "/v1/responses",
        json=_responses_opener(),
        headers={"x-acr-agent": "codex"},
    )
...
def test_invalid_provider_payload_is_dropped_with_warning(
    *,
    ...
    monkeypatch.setenv("ACR_TEST_KEY", "tier-key")
    _write_config_tree(tmp_path, CONFIG_YAML)
    payload = _provider_payload("codex", upstream="https://api.openai.com")
    ...
    app = create_app(transport=httpx.MockTransport(upstream.handler))
    with TestClient(app) as test_client:
        response = test_client.post("/v1/responses", json=_responses_opener())
...
def test_assembled_routes_signature_changes_when_provider_file_is_deleted(
    *,
    ...
    config.provider_config_path("codex").unlink()
...
def test_assemble_routes_fail_open_drops_unattributed_provider_error(
    ...
    assert server_mod._assemble_routes_fail_open({}, {"codex": {}}) == {"agents": {}}
    assert fake.calls == [{"codex"}, set()]
