...
def _chat_tool_result_body(tool_name: str = "exec_command") -> dict[str, object]:
...
def test_chat_completions_oauth_tier_routes_via_responses_backend(
    *,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    upstream: _Upstream,
    ...
    with _client_for_config(
        ...
        monkeypatch=monkeypatch,
        upstream=upstream,
        config_yaml=CHAT_OAUTH_CONFIG_YAML,
    ) as test_client:
        response = test_client.post(
            "/v1/chat/completions",
            json=_chat_tool_result_body(),
            headers={
                "authorization": "Bearer chatgpt-oauth",
                "chatgpt-account-id": "acct_chat",
            },
        )
        snapshot = metrics_mod.get_metrics().snapshot()
    ...
    assert payload["choices"][0]["message"]["content"] == "codex routed"
    # Hermes cheap turn served on the ChatGPT backend with the client's OAuth.
    assert fake.calls[0]["credential"] == "oauth"
    assert fake.calls[0]["auth_mode"] == "oauth"
    headers = {key.lower(): value for key, value in fake.calls[0]["chatgpt_headers"]}
    assert headers["authorization"] == "Bearer chatgpt-oauth"
    assert headers["chatgpt-account-id"] == "acct_chat"
    # The chat request was converted to a Responses body before dispatch.
    assert "input" in fake.calls[0]["body"]
    assert upstream.requests == []
    assert snapshot["last_requests"][0]["path"] == "/v1/chat/completions"
    assert snapshot["last_requests"][0]["route"] == "oauth_responses"
