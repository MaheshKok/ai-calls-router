...
def adapter_for_path(path: str) -> ClientAdapter | None:
    """Return the client adapter for a request path.

    Args:
        path: Incoming request URL path.

    Returns:
        The Anthropic Messages adapter for `/v1/messages`, the OpenAI Chat
        adapter for `/v1/chat/completions`, the OpenAI Responses adapter for
        `/v1/responses`, otherwise `None`.
    """
    if path == "/v1/messages":
