...
def _tool_call_delta(tool_call: dict[str, Any], index: int) -> dict[str, Any]:
...
def synthesize_chat_sse(anthropic_body: dict[str, Any]) -> Iterator[bytes]:
    ...
    yield _sse_data(
        _chunk(
            response_id=response_id, model=model, delta={}, finish_reason=choice["finish_reason"]
        )
    )
    yield b"data: [DONE]\n\n"
