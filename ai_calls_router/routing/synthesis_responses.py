...
RawEvent = tuple[str, dict[str, Any]]
...
def _event(event_type: str, sequence_number: int, **fields: Any) -> dict[str, Any]:
    """Build a Responses stream event payload."""
    return {"type": event_type, "sequence_number": sequence_number, **fields}


def _raw_event(event_type: str, **fields: Any) -> RawEvent:
    """Build an event before sequence numbering."""
    return event_type, fields


def _base_response(response: dict[str, Any], status: str) -> dict[str, Any]:
...
def _message_events(
    *,
    ...
    added_item = {"id": item_id, "type": "message", "role": "assistant", "content": []}
    part = {"type": "output_text", "text": ""}
    final_part = {"type": "output_text", "text": text}
    final_item = {**added_item, "content": [final_part]}
...
def _output_events(response: dict[str, Any]) -> Iterator[RawEvent]:
...
def _numbered_events(response: dict[str, Any]) -> Iterator[tuple[str, dict[str, Any]]]:
...
def synthesize_responses_sse(anthropic_body: dict[str, Any], model: str) -> Iterator[bytes]:
